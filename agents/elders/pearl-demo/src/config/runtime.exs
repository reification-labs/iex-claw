...
if config_env() == :dev do
  ...
  # Allow binding to all interfaces in Docker (default: 127.0.0.1 in dev.exs)
  # Set PHX_IP=0.0.0.0 to allow connections from outside the container
  if phx_ip = System.get_env("PHX_IP") do
    ip_tuple =
      phx_ip
      |> String.split(".")
...
if config_env() == :prod do
  ...
  # Fix Fly.io postgres URLs: .flycast is deprecated, use .internal
  database_url = String.replace(database_url, ".flycast", ".internal")
  ...
  config :reify, ReifyWeb.Endpoint,
    ...
    # comes from the same host. This helps prevent CSRF and is appropriate when
    # running behind Fly.io's load balancing.
    check_origin: :conn,
    http: [
