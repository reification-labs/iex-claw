...
export default defineConfig(({ command }) => {
  ...
  return {
    ...
    publicDir: false, // no assets/static directory
    server: {
      // In VS Code devcontainers/Codespaces (when opened in VS Code Desktop),
      // the editor handles port forwarding so both Phoenix and Vite appear as
      // localhost to the browser, and no additional CORS/origin config is needed.
      // For standalone Docker/docker-compose or any setup where Phoenix and Vite
      // truly run on different origins, you may still need explicit CORS/origin
      // configuration (see docker-compose and Phoenix endpoint settings).
    },
    plugins: [
      react(),
      liveReactPlugin(),
